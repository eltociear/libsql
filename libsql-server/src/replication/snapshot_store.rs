...
impl SnapshotStoreInner {
    ...
    pub fn delete_all(&self, namespace: &NamespaceName) -> Result<()> {
        ...
        conn.execute("DELETE FROM snapshots WHERE namespace = ?", [namespace.as_str()])?;
...
impl SnapshotStore {
    pub async fn new(base_path: &Path) -> Result<Self> {
        ...
        let conn =
            open_conn_enable_checkpoint(&store_path, Sqlite3WalManager::default(), None, 1000, None)?;
    ...
    pub async fn find_file(
        ...
        match self.find(namespace, frame_no)? {
            Some(snapshot_id) => {
                let path = self.inner.snapshots_path(namespace).join(snapshot_id.to_string());
